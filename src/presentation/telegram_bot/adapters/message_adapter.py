...
class MessageAdapter:
    ...
    def _format_complete(
        ...
        lines.append(f"{icon} **{vehicle_desc if vehicle_desc else 'Vehicle Information'}**")
        lines.append("=" * 30)
        ...
        if is_autodev:
            ...
            self._add_field(lines, "Engine Manufacturer", attrs.get("engine_manufacturer"))
        ...
        if not is_autodev:
            ...
            if attrs.get("weight_empty_kg"):
                self._add_field(lines, "Empty Weight", f"{attrs.get('weight_empty_kg')} kg")
        ...
        if is_autodev:
            ...
            if attrs.get("max_speed_kmh"):
                self._add_field(lines, "Max Speed", f"{attrs.get('max_speed_kmh')} km/h")
            if attrs.get("avg_co2_emission_g_km"):
                self._add_field(lines, "CO2 Emission", f"{attrs.get('avg_co2_emission_g_km')} g/km")
    ...
    def format_error_message(self, error: str, vin: Optional[str] = None) -> str:
        ...
        lines.append(error)
        lines.append("")
        lines.append("Please check the VIN and try again, or contact support if the issue persists.")
    ...
    def format_welcome_message(
        ...
        lines.append("")
        lines.append("I can decode Vehicle Identification Numbers (VINs) using official databases.")
        lines.append("")
