...
class NHTSAClient(VINDecoderBase):
    ...
    def __init__(self, api_key: Optional[str] = None, cache=None, timeout: int = 15):
        ...
        Note: NHTSA API doesn't require an API key
        """
        super().__init__(api_key=None, cache=cache, timeout=timeout)
    ...
    async def decode_vin(self, vin: str) -> Dict[str, Any]:
        ...
        # NHTSA API endpoint
        url = f"{self.BASE_URL}/DecodeVin/{vin.upper()}?format=json"
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if not data.get("Results"):
                    raise NHTSAError("No results returned from NHTSA API")
            ...
            raise NHTSAError(f"NHTSA API error: {e.response.status_code}")
        except httpx.RequestError as e:
            logger.error(f"NHTSA API request error: {e}")
            raise NHTSAError(f"Network error: {str(e)}")
        except Exception as e:
            logger.error(f"Unexpected error in NHTSA decode: {e}")
            raise NHTSAError(f"Failed to decode VIN: {str(e)}")
